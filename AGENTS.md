...
本节沉淀跨任务的通用工程约定，适用于所有开发与维护操作。与上述项目特定规则（配色、文章系统、标题命名等）并列生效。

### 8.1 安全边界

- **禁止执行危险命令**：不得执行 `rm -rf /`、`rm -rf ~`、`del /f /s /q C:\` 等递归删除根目录或用户目录的命令；删除操作必须限定到具体目标路径
- **禁止强制推送覆盖远端**：未经用户明确确认，不得执行 `git push --force` / `git push -f`，避免覆盖远端已有提交
- **敏感操作二次确认**：涉及删除大量文件、重置 git 历史、修改 `.gitignore` 忽略已跟踪文件、批量重命名、修改 CI/CD 配置等不可逆操作前，必须向用户说明影响范围并等待确认
- **p 目录保护**：详见 Section 1，常规开发不主动修改 `p/` 目录业务逻辑代码

### 8.2 沟通与反馈规范

- **报错根因优先**：遇到错误时，先分析并给出根因（为什么会出错），再给出修复方案（怎么解决）；避免只贴错误信息不分析，或只给修复不给原因
- **架构变更影响面分析**：涉及目录结构调整、CSS 变量改动、文章系统结构变更、构建/部署方式变化等架构级改动时，必须输出影响面分析：影响哪些文件/功能、是否有破坏性变更、向前兼容性评估
- **技术讨论语言**：技术讨论与说明使用中文；代码中的变量名、函数名、CSS 类名、文件名使用英文（遵循现有项目惯例）
- **假设显式化**：当基于合理假设行动时，在回复中简要说明所做的假设，便于用户发现偏差并及时纠正

### 8.3 工具与命令约定

- **允许的工具**：`git`、`npm`、`node`、`python`（用于本地预览服务器）、`7z`（解压）
- **文件操作优先专用工具**：搜索文件用 Glob，搜索内容用 Grep，读取用 Read，编辑用 SearchReplace；避免用 PowerShell 的 `find`/`grep`/`cat`/`sed`/`awk` 等命令处理项目文件
- **外部信息获取**：通过 WebSearch / WebFetch 获取参考资料时，优先选择权威来源（官方文档、RFC、厂商发布、权威媒体），并在回复中标注引用出处
- **长时运行命令**：启动本地预览服务器、watch 进程等长时运行任务时，使用非阻塞执行（blocking: false），避免阻塞与用户的交互